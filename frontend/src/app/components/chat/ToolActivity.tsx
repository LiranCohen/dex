...
export const ToolActivity = memo(function ToolActivity({
  ...
  return (
    <div className={`app-tool-activity app-tool-activity--${status} ${hasError ? 'app-tool-activity--has-error' : ''}`}>
      {/* Header row */}
      <button
        type="button"
        onClick={() => !isRunning && hasDetails && setIsExpanded(!isExpanded)}
        disabled={isRunning || !hasDetails}
        className="app-tool-activity__header"
      >
        <div className="app-tool-activity__left">
          <span className="app-tool-activity__icon">{info.icon}</span>
          <span className="app-tool-activity__label">{info.label}</span>
          {isRunning ? (
            <span className="app-tool-activity__pulse" />
          ) : hasError ? (
            <span className="app-tool-activity__status app-tool-activity__status--error">✗</span>
          ) : (
            <span className="app-tool-activity__status app-tool-activity__status--success">✓</span>
          )}
        </div>

        <div className="app-tool-activity__right">
          {!isRunning && durationMs != null && durationMs > 0 && (
            <span className="app-tool-activity__duration">{durationMs}ms</span>
          )}
          {!isRunning && hasDetails && (
            <span className="app-tool-activity__expand">{isExpanded ? '▲' : '▼'}</span>
          )}
        </div>
      </button>

      {/* Summary row */}
      <div className="app-tool-activity__summary">
        <span className={hasError ? 'app-tool-activity__summary--error' : ''}>
          {displayText}
        </span>
      </div>
      ...
      {isExpanded && !isRunning && hasDetails && (
        <div className="app-tool-activity__details">
          {input && (
            <div className="app-tool-activity__section">
              <span className="app-tool-activity__section-label">Input:</span>
              <pre className="app-tool-activity__code">
          ...
          {output && (
            <div className="app-tool-activity__section">
              <span className="app-tool-activity__section-label">Output:</span>
              <pre className={`app-tool-activity__code app-tool-activity__output ${hasError ? 'app-tool-activity__output--error' : ''}`}>
                {output || '(empty)'}
              </pre>
            </div>
          )}
