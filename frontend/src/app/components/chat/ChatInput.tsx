...
export function ChatInput({
  ...
  return (
    <div className="app-chat-input-wrapper">
      <div
        ref={containerRef}
        className={`app-chat-input ${connectionStatus !== 'connected' ? `app-chat-input--${connectionStatus}` : ''}`}
      >
        {/* Connection status indicator */}
        {connectionStatus !== 'connected' && (
          <span
            className={`app-chat-input__status app-chat-input__status--${connectionStatus}`}
            aria-live="polite"
        ...
        <span className="app-chat-input__cursor">▌</span>
        <TextareaAutosize
          ref={textareaRef}
          className="app-chat-input__field"
          value={value}
          onChange={(e) => {
        ...
        {showStop ? (
          <button
            type="button"
            className="app-btn app-btn--secondary app-chat-input__stop"
            onClick={handleStop}
          ...
          <button
            type="button"
            className="app-chat-input__send"
            onClick={handleSend}
            disabled={!canSend}
            aria-label="Send message"
          >
            <svg className="app-chat-input__send-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor">
      ...
      <div
        id="chat-input-hint"
        className={`app-chat-input__hint ${connectionStatus !== 'connected' ? `app-chat-input__hint--${connectionStatus}` : ''}`}
