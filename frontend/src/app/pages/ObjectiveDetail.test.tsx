...
describe('ObjectiveDetail', () => {
  ...
  describe('checklist', () => {
    ...
    it('shows completed status for done items', async () => {
      ...
      const doneItem = screen.getByText('Create login endpoint').closest('.app-checklist-item');
      expect(doneItem).toBeInTheDocument();
      expect(doneItem?.querySelector('.app-checklist-item__icon--complete')).toBeInTheDocument();
    });
    ...
    it('shows pending status for incomplete items', async () => {
      ...
      const pendingItem = screen.getByText('Implement auth middleware').closest('.app-checklist-item');
      expect(pendingItem?.querySelector('.app-checklist-item__icon--pending')).toBeInTheDocument();
    });
  ...
  describe('status indicators', () => {
    it('shows active status bar for running tasks', async () => {
      ...
      expect(document.querySelector('.app-status-bar--active')).toBeInTheDocument();
    });
    ...
    it('shows pulsing animation for running tasks', async () => {
      ...
      expect(document.querySelector('.app-status-bar--pulse')).toBeInTheDocument();
    });
  });
  ...
  describe('header', () => {
    it('shows back link to quest when task has QuestID', async () => {
      ...
      expect(backLink).toHaveAttribute('href', '/quests/quest-1');
    });
