...
describe('StatusBar', () => {
  it('renders with active status', () => {
    ...
    expect(statusBar).toHaveClass('v2-status-bar--active');
  ...
  it('renders with pending status', () => {
    ...
    const statusBar = screen.getByRole('status');
    expect(statusBar).toHaveClass('v2-status-bar--pending');
  ...
  it('renders with complete status', () => {
    ...
    const statusBar = screen.getByRole('status');
    expect(statusBar).toHaveClass('v2-status-bar--complete');
  ...
  it('renders with error status', () => {
    ...
    const statusBar = screen.getByRole('status');
    expect(statusBar).toHaveClass('v2-status-bar--error');
  ...
  it('applies pulse animation when status is active and pulse is true', () => {
    ...
    const statusBar = screen.getByRole('status');
    expect(statusBar).toHaveClass('v2-status-bar--pulse');
  });

  it('does not apply pulse animation when status is not active', () => {
    render(<StatusBar status="pending" pulse />);
    const statusBar = screen.getByRole('status');
    expect(statusBar).not.toHaveClass('v2-status-bar--pulse');
  });
  ...
  it('applies custom className', () => {
    ...
    expect(statusBar).toHaveClass('v2-status-bar');
  });

  it('always has base class', () => {
    render(<StatusBar status="complete" />);
    expect(screen.getByRole('status')).toHaveClass('v2-status-bar');
  });
});
