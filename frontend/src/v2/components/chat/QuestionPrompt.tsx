...
export function QuestionPrompt({
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (!hasFocus) {
        const allUnansweredQuestions = document.querySelectorAll('.v2-question:not(.v2-question--answered)');
  ...
  return (
    <div ref={containerRef} className={`v2-question ${isAnswered ? 'v2-question--answered' : ''}`} role="group" aria-labelledby="question-text">
      <div className="v2-question__header">
        <div className="v2-question__label">
          {isAnswered ? 'Answered' : 'Question'}
        </div>
        {(timestamp || contextRef) && (
          <div className="v2-question__context">
            {contextRef && <span className="v2-question__context-ref">{contextRef}</span>}
            {timestamp && <time className="v2-question__context-time">{timestamp}</time>}
          </div>
        )}
      </div>
      <p id="question-text" className="v2-question__text">{question}</p>

      <div className="v2-question__options" role="listbox" aria-label="Answer options" aria-describedby={!disabled && !isAnswered && options.length > 0 ? "question-hint" : undefined}>
        ...
        {!disabled && !isAnswered && onCustomAnswer && (
          <>
            {!showCustomInput ? (
              <button
                type="button"
                className="v2-question__option v2-question__option--other"
                onClick={handleOtherClick}
              >
                <span className="v2-question__option-key" aria-hidden="true">...</span>
                <div>
                  <div className="v2-question__option-title">Other</div>
                  <div className="v2-question__option-desc">Type your own answer</div>
                </div>
              </button>
            ) : (
              <div className="v2-question__custom-input-container">
                <input
                  ref={customInputRef}
                  type="text"
                  className="v2-question__custom-input"
                ...
                <button
                  type="button"
                  className="v2-btn v2-btn--primary v2-question__custom-submit"
      ...
      {isCustomAnswer && (
        <div className="v2-question__custom-answer">
          <span className="v2-question__custom-label">Your answer:</span>
          <p className="v2-question__custom-text">{customAnswer}</p>
        </div>
      )}

      {!disabled && !isAnswered && options.length > 0 && (
        <p id="question-hint" className="v2-question__hint">
