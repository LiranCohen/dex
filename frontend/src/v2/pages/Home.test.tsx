...
describe('Home', () => {
  ...
  describe('loading state', () => {
    it('shows skeleton while loading', () => {
      render(<Home />);
      // SkeletonList renders skeleton elements
  ...
  describe('new quest', () => {
    ...
    it('creates new quest and navigates on click', async () => {
      ...
      await waitFor(() => {
        expect(mockNavigate).toHaveBeenCalledWith(expect.stringMatching(/\/v2\/quests\/quest-/));
      });
    });
  ...
  describe('status indicators', () => {
    it('shows active status for running quests', async () => {
      ...
      const questCard = screen.getByText('Test Quest').closest('.v2-quest-card');
      expect(questCard?.querySelector('.v2-status-bar--active')).toBeInTheDocument();
    });
    ...
    it('shows complete status for completed quests', async () => {
      ...
      const questCard = screen.getByText('Completed Quest').closest('.v2-quest-card');
      expect(questCard?.querySelector('.v2-status-bar--complete')).toBeInTheDocument();
    });
  });
  ...
  describe('keyboard navigation', () => {
    it('highlights selected quest with j/k navigation', async () => {
      ...
      await waitFor(() => {
        const selectedCard = document.querySelector('.v2-card--selected');
