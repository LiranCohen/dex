...
describe('AllObjectives', () => {
  ...
  describe('task list', () => {
    ...
    it('links tasks to their detail pages', async () => {
      ...
      await waitFor(() => {
        ...
        expect(taskLink).toHaveAttribute('href', '/v2/objectives/task-1');
      });
    });
    ...
    it('links quest titles to quest pages', async () => {
      ...
      await waitFor(() => {
        ...
        expect(questLink).toHaveAttribute('href', '/v2/quests/quest-1');
      });
    });
  ...
  describe('status indicators', () => {
    it('shows active status bar for running tasks', async () => {
      ...
      const runningTask = screen.getByText('Implement feature X').closest('.v2-objectives-list-item');
      expect(runningTask?.querySelector('.v2-status-bar--active')).toBeInTheDocument();
    });
    ...
    it('shows pending status bar for pending tasks', async () => {
      ...
      const pendingTask = screen.getByText('Pending task').closest('.v2-objectives-list-item');
      expect(pendingTask?.querySelector('.v2-status-bar--pending')).toBeInTheDocument();
    });
    ...
    it('shows complete status bar for completed tasks', async () => {
      ...
      const completedTask = screen.getByText('Completed task').closest('.v2-objectives-list-item');
      expect(completedTask?.querySelector('.v2-status-bar--complete')).toBeInTheDocument();
    });
  });
